...
stringifyLocation : Location -> String
stringifyLocation (x, y) =
...
main =
    let
        finalLocation =
            calculateFinalLocation inputData
                |> stringifyLocation
            
        firstIntersection =
            calculateFirstIntersection inputData
                |> Maybe.map stringifyLocation
                |> Maybe.withDefault "No intersection found"
    in
    Html.div
        []
        [ Html.p []
            [ Html.text ("Final Location: " ++ finalLocation)
            ]
        , Html.p []
            [ Html.text ("First Intersection: " ++ firstIntersection)
            ]
        ]

{-| Which is the final location?
...
calculateFinalLocation : String -> Location
calculateFinalLocation instructions =
    String.foldl move (0, 0) instructions
    
    
{-| Which is the first location visited twice?

Example inputs:
    >>vv     -> Will return Nothing since we never visit the same
                location twice

    ><>>>>   -> You will revisit { x = 0, y = 0 } on the second
                instruction.
    ^^<<v>>> -> You will walk in a square and revisit
               { x = 0, y = 1 } on the 7th instruction.
-}
{-
Save positions that we already visited. Check every time if that position is already visited
-}
calculateFirstIntersection : String -> Maybe Location
calculateFirstIntersection instructions =
 let
        -- Helper function 
        go : Location -> List Location -> List Char -> Maybe Location
        go currentPos visited remainingInstructions =
            case remainingInstructions of
                [] ->
                    Nothing  -- If no instructions left, return Nothing
                
                direction :: remainingInstructionsRest ->
                    let
                        newPos = move direction currentPos
                    in
                    if List.member newPos visited then
                        Just newPos  -- If we have already visited this position, return it as the intersection
                    else
                        go newPos (newPos :: visited) remainingInstructionsRest  -- Continue to the next instruction
    in
    -- Start from the initial position (0, 0) with an empty list of visited locations
    go (0, 0) [(0, 0)] (String.toList instructions)